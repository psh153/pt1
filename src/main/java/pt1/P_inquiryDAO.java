...
public class P_inquiryDAO extends OracleServer {
	...
	public ArrayList<P_inquiryDTO> getP_inquiry(int pnum, int start, int end) {
		ArrayList<P_inquiryDTO> inquiryList = null;
		try {
			conn = getConnection();
	...
	// 답변만 불러오는 메서드
	public ArrayList<P_inquiryDTO> answerInquiry(int ref) {
		ArrayList<P_inquiryDTO> list = new ArrayList<>();
		P_inquiryDTO dto = null;
		try {
			conn = getConnection();
	...
	// 내 문의글을 불러오는 메서드
	public ArrayList<P_inquiryDTO> getMyp_Inquiry(String id, int start, int end) {
		ArrayList<P_inquiryDTO> myInquiryList = null;
		try {
			conn = getConnection();
			sql = "select num,pnum,writer,subject,reg,ref,ref_step, content,status,brand,pname, r "
					+ "from (select num,pnum,writer,subject,reg,ref,ref_step,content,status,brand,pname, rownum r "
