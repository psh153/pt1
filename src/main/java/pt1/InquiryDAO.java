...
public class InquiryDAO extends OracleServer {
	...
	public ArrayList<InquiryDTO> getArticles(int start, int end) {
		ArrayList<InquiryDTO> articleList = null;
		try {
			conn = getConnection();
	...
	// 내 문의글을 불러오는 메서드
	public ArrayList<InquiryDTO> getMyInquiry(String id, int start, int end) {
		ArrayList<InquiryDTO> articleList = new ArrayList<>();
		try {
			conn = getConnection();
	...
	// 답변 불러오는 메서드
	public ArrayList<InquiryDTO> answerArticle(int ref) {
