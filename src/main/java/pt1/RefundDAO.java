...
public class RefundDAO extends OracleServer{
	...
	public void deleteRefund(String user_id, int product_id) {
		try {
			conn = getConnection();
			sql = "delete from PAYMENT_HISTORY where user_id=?, product_id = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public ArrayList<RefundDTO> getRefund(String user_id) {
		ArrayList<RefundDTO> mmylist=null;
		try {
			conn = getConnection();
	...
	public RefundDTO checkRefund(String user_id, int product_id) {
		...
		try {
			conn = getConnection();
			sql = "select * from refund where user_id=? , product_id = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, user_id);
			pstmt.setInt(2, product_id);
			rs = pstmt.executeQuery();
	...
	public int RefundCheck(String user_id, int product_id) {// 리턴타입 int임
		...
		try {
			...
			pstmt = conn.prepareStatement("select pw from REFUND where user_id = ?,product_id=?"); // 쿼리문작성중. 멤버 태그에 있는 비밀번호만 검색하겠다는거임.
	...
	public ArrayList<RefundDTO> findRefund(String user_id, int Payment_id) {
		ArrayList<RefundDTO> refundlist=null;
		try {
			conn = getConnection();
